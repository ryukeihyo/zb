...
class UserController extends HomeController{
    ...
    public function login(){
        if(IS_POST){
			...
			$res=$user_object->where("username='".$username."'")->find();
			
			if($res['usertype']==6){
				
				$this->error('你没有权限！', Cookie('__forward__') ? : C('HOME_PAGE'));
			}
    ...
    public function resetPassword(){
        ...
        if(IS_POST){
		   ...
		   if($uinfo['password']!=user_md5(I('post.oldpassword'))){
            ...
            $result = $user_object->save($data); //重置密码
			$username=$this->__USER__['username'];
            $uid = $user_object->login($username, I('post.password')); //自动登录
            if($uid){
                $this->success('密码修改成功', C('HOME_PAGE'));
            }else{
                $this->error('密码修改失败');
            }
        }else{
	...
	public function checkpassword(){
		...
		$uinfo=D("User")->where("mobile='".$_POST['uname']."' or username='".$_POST['uname']."'")->find();
		

	   
	   if($uinfo['password']!=user_md5(I('post.password'))){
