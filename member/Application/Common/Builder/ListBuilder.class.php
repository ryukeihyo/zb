    ...
    public function addRightButton($type, $attribute = null){
        switch($type){
            ...
            case 'member':
                //预定义按钮属性以简化使用
                $my_attribute['class'] = 'label label-default';
				$my_attribute['href']  = U(MODULE_NAME.'/'.CONTROLLER_NAME.'/memberlist', array($this->_table_data_list_key => '__data_id__'));
            ...
            case 'yeji':
                //预定义按钮属性以简化使用
                $my_attribute['class'] = 'label label-default';
				$my_attribute['href']  = U(MODULE_NAME.'/'.CONTROLLER_NAME.'/yeji', array($this->_table_data_list_key => '__data_id__'));
                //如果定义了属性数组则与默认的进行合并
                if($attribute){
                    $my_attribute = array_merge($my_attribute, $attribute);
                }else{
                    $my_attribute['title'] = '业绩列表';
                }
