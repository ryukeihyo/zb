...
class UserController extends AdminController{
    ...
    public function index(){
        ...
        $builder->setMetaTitle('用户列表') //设置页面标题
               ...
               // ->addTableColumn('money', '余额')
                ...
                ->addTableColumn('isyucun', '开启预存', 'status')
    ...
    public function merchant(){
        ...
        //获取所有用户
        $map['status'] = array('egt', '0'); //禁用和正常状态
        $map['usertype'] = array('eq', '5'); //禁用和正常状态
        $data_list = D('User')->page(!empty($_GET["p"])?$_GET["p"]:1, C('ADMIN_PAGE_ROWS'))->where($map)->order('sort desc,id desc')->select();
        ...
        //使用Builder快速建立列表页面。
        $builder = new \Common\Builder\ListBuilder();
        $builder->setMetaTitle('商家会员') //设置页面标题
                ->addTopButton('addnew')  //添加新增按钮
                ->addTopButton('resume')  //添加启用按钮
                ->addTopButton('forbid')  //添加禁用按钮
                ->addTopButton('delete')  //添加删除按钮
                ->setSearch('请输入ID/商家编号/邮箱/手机号', U('index'))
                ->addTableColumn('id', 'UID')
               // ->addTableColumn('usertype', '类型')
                ->addTableColumn('username', '商家编号')
                ->addTableColumn('platform', '平台名称')
                ->addTableColumn('realname', '姓名')
                ->addTableColumn('mobile', '手机号')
                ->addTableColumn('zebaib', '泽百币')
                ->addTableColumn('yucun', '预存')
               // ->addTableColumn('vip', 'VIP')
               // ->addTableColumn('score', '积分')
               // ->addTableColumn('money', '余额')
                ->addTableColumn('last_login_time', '最后登录时间时间', 'time')
                ...
                ->addRightButton('forbid') //添加禁用/启用按钮
                ->addRightButton('delete') //添加删除按钮
                ->addRightButton('member') //添加删除按钮
                ->addRightButton('yeji') //添加删除按钮
                ->display();
    }
    ...
    public function individual(){
        ...
        //获取所有用户
        $map['status'] = array('egt', '0'); //禁用和正常状态
        $map['usertype'] = array('eq', '6'); //禁用和正常状态
        $data_list = D('User')->page(!empty($_GET["p"])?$_GET["p"]:1, C('ADMIN_PAGE_ROWS'))->where($map)->order('sort desc,id desc')->select();
    ...
    public function memberlist(){
        ...
        //获取所有用户
        $map['status'] = array('egt', '0'); //禁用和正常状态
        $map['usertype'] = array('eq', '6'); //
        $map['parentid'] = array('eq', $parentid); //商户ID
        $data_list = D('User')->page(!empty($_GET["p"])?$_GET["p"]:1, C('ADMIN_PAGE_ROWS'))->where($map)->order('sort desc,id desc')->select();
    ...
    public function yeji(){
        ...
        //获取所有用户
        $map['parentid'] = array('eq', $parentid); //商户ID
        $data_list = D('Xiaofei')->page(!empty($_GET["p"])?$_GET["p"]:1, C('ADMIN_PAGE_ROWS'))->where($map)->order('id desc')->select();

		//查询被赠送积分的会员信息
		foreach($data_list as $key=>$val){
			if($val['uid']){
				$us=D("User")->where("id=".$val['uid'])->find();
				$data_list[$key]['kahao']=$us['username'];
				$data_list[$key]['xingming']=$us['realname'];
			}
		}

		
		
        $page = new \Common\Util\Page(D('Xiaofei')->where($map)->count(), C('ADMIN_PAGE_ROWS'));
        ...
        $builder->setMetaTitle('商家会员') //设置页面标题
               ...
               // ->addTopButton('delete')  //添加删除按钮
               // ->setSearch('请输入ID/用户名/邮箱/手机号', U('index'))
                ->addTableColumn('id', '序号')
                ->addTableColumn('realname', '商家名称')
                ->addTableColumn('username', '商家编号')
                ->addTableColumn('kahao', '会员卡号')
                ->addTableColumn('xingming', '会员姓名')

                ->addTableColumn('fmoney', '消费金额')
                ->addTableColumn('jifen', '赠送积分')
                ->addTableColumn('addtime', '消费时间', 'time')
                 ->setTableDataList($data_list) //数据列表
                ->setTableDataPage($page->show()) //数据列表分页
                ->display();
    }
        ...
        if(IS_POST){
            ...
            $builder->setMetaTitle('新增用户') //设置页面标题
                    ...
                    ->addFormItem('mobile', 'text', '手机号码', '手机号码')
					 ->addFormItem('isyucun', 'radio', '开启预存', '是否开启预存',array(0=>'关闭',1=>'开启'))
                    ->addFormItem('realname', 'text', '省市区县', '省市区县')
                    ->addFormItem('password', 'password', '密码', '密码')
                    ->addFormItem('rate', 'text', '赠送比例', '赠送比例')
                    ->addFormItem('guanlifei', 'text', '管理费', '管理费比例')
    ...
    public function edit($id){
        ...
        if(IS_POST){
            ...
            $builder->setMetaTitle('编辑用户') //设置页面标题
                    ...
                    ->addFormItem('email', 'text', '邮箱', '邮箱')
                    ->addFormItem('mobile', 'text', '手机号码', '手机号码')
                    ->addFormItem('zebaib', 'text', '泽百币', '泽百币')
                    ->addFormItem('yucun', 'text', '预存', '预存')
                    ->addFormItem('isyucun', 'radio', '开启预存', '是否开启预存',array(0=>'关闭',1=>'开启'))
